...
import courseclock.timetable.base_view.BaseTitleActivity
import es.dmoral.toasty.Toasty
import courseclock.timetable.databinding.ActivityTimeSettingsBinding
...
class TimeSettingsActivity : BaseTitleActivity() {
    ...
    private fun initView() {
        binding = ActivityTimeSettingsBinding.bind(findViewById<ViewGroup>(android.R.id.content).getChildAt(0))
...
//        })
    ...
    private fun exitBy2Click() {
        if (!isExit) {
            isExit = true // 准备退出
            binding.llRoot.longSnack("真的不保存吗？那再按一次退出编辑哦，就不保存啦。")
            launch {
                delay(2000)
                isExit = false
            }
